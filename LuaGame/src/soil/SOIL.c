// This is a modified version of SOIL. The current official release uses glGetString(GL_EXTENSIONS), which is deprecated and broken.
...
#define SOIL_AVAILABLE 1
#define SOIL_UNAVAILABLE NULL

int SOIL_query_gl_extension(const char * extension_name) {
	int extcount;

	glGetIntegerv(GL_NUM_EXTENSIONS, &extcount);

	for (int ext = 0; ext < extcount; ext++)
		if (strcmp(glGetStringi(GL_EXTENSIONS, ext), extension_name) == 0)
			return SOIL_AVAILABLE;

	return SOIL_UNAVAILABLE;
}
...
int query_NPOT_capability( void )
{
	...
	if( has_NPOT_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		if (!SOIL_query_gl_extension("GL_ARB_texture_non_power_of_two"))
		{
...
int query_tex_rectangle_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if(	(!SOIL_query_gl_extension("GL_ARB_texture_rectangle")) &&
			(!SOIL_query_gl_extension("GL_EXT_texture_rectangle")) &&
			(!SOIL_query_gl_extension("GL_NV_texture_rectangle")))
		{
			/*	not there, flag the failure	*/
...
int query_cubemap_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if(	(!SOIL_query_gl_extension("GL_ARB_texture_cube_map")) &&
			(!SOIL_query_gl_extension("GL_EXT_texture_cube_map")))
		{
			/*	not there, flag the failure	*/
...
int query_DXT_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if((!SOIL_query_gl_extension("GL_EXT_texture_compression_s3tc")))
		{
			/*	not there, flag the failure	*/
